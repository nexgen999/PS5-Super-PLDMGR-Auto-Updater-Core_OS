...
def build_apps_package():
    timestamp = datetime.now().strftime("%Y.%m.%d-%H%M")
    os.makedirs("archives", exist_ok=True)
    json_path = "json/apps.json"
    
    print(f"=== Création du package AIO Apps ({timestamp}) ===")
    ...
    try:
        ...
        zip_filename = f"archives/PS5_apps_aio_latest.zip"
        zip_versioned = f"archives/PS5_apps_aio_{timestamp}.zip"
        ...
        print(f"✅ Archive Apps créée : {zip_filename}")
    except Exception as e:
